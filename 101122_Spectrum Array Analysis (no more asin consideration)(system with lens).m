...
%若系統有lens, 則無法用TD的最大值當作TD中心, 因為在不同深度收光量(DC)本來就不同, 只好先假設TD和FD共心


N_f=4096*8;
N_t=N_f*8;

data_o=importdata('D:\101129_1k 0.1-3.txt');
data(1:fix(size(data_o,1)/20),1:size(data_o,2))=0;
for j=1:fix(size(data_o,1)/20)
    data(j,:)=data_o(20*j,:);
end
...
%data_array(j,:)=data_array(j,:)-data_array(end,:);
%end
%% TD data calculation
...
length_space_FD=round(length(time)/5);
...
%%  Interfered PSF FWHM (revised!)
space_min_for_inter_peak_FD=abs(space_TD(j)/2);  %要求well resolved
