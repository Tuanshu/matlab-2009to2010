...
N_lambda=4096*4;    %zero padding
N_f=4096*4;         %interpolation
N_t=N_f;            %fft
...
pixel=[1:4096]';  %1~4096
[peak index_peak]=max(data_array_o(1,:));
...
lambda_o=grating_pitch*sin((atan((index_peak-pixel)*Q_o)+incidence_angle))+center_wavelength/2;   %atan!
...
data_array=abs(ifft(ifftshift(data_array_fft_temp),N_lambda,2));
