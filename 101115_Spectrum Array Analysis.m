...
data_o=importdata('D:\101116_long range spectrum array (10k)_adj_1.txt');
data(1:round(size(data_o,1)/10),1:size(data_o,2))=0;
for j=1:round(size(data_o,1)/10)
    data(j,:)=data_o(10*j,:);
end
clear data_o;
%data=data(750:1750,:);
...
%%  Interfered PSF FWHM
space_min_for_inter_peak_FD=space_FD(1)+1.5*FWHM_DC_FD;  %要求well resolved
