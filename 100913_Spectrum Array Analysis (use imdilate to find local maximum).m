...
data=importdata('D:\100913_set3_1kHz_0.01mms_325mA_(9.68to9.74).txt');
data=data(750:1750,:);
...
dilate_envelope=imdilate(CS_envelope(:,j),ones(find(CS_envelope(:,j)<0.5*value_DC_FD, 1, 'first')));           %如果干涉效率太低&&envelope太醜這裡會出錯
peak_index=find(CS_envelope(:,j)==dilate_envelope); %注意! 此處的inter_peakindex_FD會抓錯, 因為CS_envelope從space_min_for_inter_peak_index_FD開始而非0開始
peakvalues=sort(CS_envelope(peak_index,j),'descend');
inter_peakvalue_FD=peakvalues(2);   %1應該是DC
inter_position_index_FD=find(CS_envelope(:,j)==peakvalues(2),1, 'first');
inter_position_FD(j)=space_FD(inter_position_index_FD);
FWHM_FD_right=space_FD(inter_position_index_FD-1+find(CS_envelope(inter_position_index_FD:round(length(CS_envelope(:,j))/2),j)<0.5*inter_peakvalue_FD, 1, 'first'));
FWHM_FD_left=space_FD(find(CS_envelope(1:inter_position_index_FD,j)<0.5*inter_peakvalue_FD, 1, 'last'));
