...
data_array_o=data(1:500,2:end);
time_record=data(1:500,1);

N_lambda=4*4096;    %zero padding
...
lambda_o=grating_pitch*sin((atan((index_peak_o-pixel)*Q_o)+incidence_angle))+center_wavelength/2;   %atan!
...
%% Zero padding
data_array_fft_temp=fft(data_array_o,[],2);
data_array=N_lambda/length(lambda_o)*abs(ifft(ifftshift(data_array_fft_temp),N_lambda,2)); %不知道為什麼這裡用real而非abs SNR會比較高
...
Q=(long_coef-short_coef)/(index_long-index_short);      %roughly flens/pixel size
lambda=grating_pitch*sin((atan((index_peak-pixel_n)*Q)+incidence_angle))+center_wavelength/2;   %atan!
...
freq=[ZZ;freq];
data_array=[zeros(500,length(ZZ)) data_array];
d_f=max(freq)/(N_f-1);
fx=[0:d_f:max(freq)]';
d_t=1/(d_f*N_t);
%d_t=1/(d_f*2*N_f);
...
SNR=20*log10(mean(peak_CS)/RMS_total);
